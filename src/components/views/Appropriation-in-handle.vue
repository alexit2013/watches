              ...
              <div class="un-handle-msg">
                ...
                <div class="msg-main">
                  ...
                  <div class="main-message">
                    <p><span>申请对象：</span><span>{{appropriationInRecord.obj.userName}}</span></p>
                    <p><span>申请时间：</span><span>{{appropriationInRecord.obj.subTime}}</span></p>
                    <p><span>申请原因：</span><span>{{appropriationInRecord.obj.describe}}</span></p>
...
<script>
  ...
  export default {
    ...
    filters: {
      filterRecord(id) {
        console.log(id);
        let circleList = JSON.parse(localStorage.getItem('appropInHandle' + sessionStorage.getItem('userRecordId')));
    ...
    methods: {
      // 查看记录详情
      appropRecord(id) {
        let list = JSON.parse(localStorage.getItem('appropInHandle' + sessionStorage.getItem('userRecordId')));
        console.log(list);
        if (list == null) {
          list = [];
        }
        ...
        localStorage.setItem('appropInHandle' + sessionStorage.getItem('userRecordId'), JSON.stringify(list));
        let a1 = [];
        for (let item of this.appropriationInHandleList) {
          a1.push(item.id);
        };
        let a2 = JSON.parse(localStorage.getItem('appropInHandle' + sessionStorage.getItem('userRecordId')));
        // console.log(a1);
        // console.log(a2);
        if (a1.every(val => a2.includes(val))) {
          this.$emit('signProp', 0);
        } else {
          this.$emit('signProp', 1);
        }
      ...
      getList() {
        ...
        this.list = this.appropriationInHandleList.filter((item, index) =>
