    ...
    <div v-show="completedRecordList.length !== 0">
      <table v-for="(item,index) in list" :key="index" class="borrow-table">
        <tr>
          <th>对象</th>
          <th>原因/描述</th>
          <th>{{item.type == 1 ? '打款时间' : (item.type == 2 ? '打款时间' : '转账时间')}}</th>
          <th>金额</th>
          <th>操作
            {{item.id | filterRecord}}
          </th>
        </tr>
        <tr>
          <td>{{item.userNick}}</td>
          ...
          <td>
            ...
            <el-dialog title="记录详情" v-if="dialogCompletedRecordDetailsVisible == true"
              :visible.sync="dialogCompletedRecordDetailsVisible" center>
              <div>
              ...
              <div slot="footer">
                <el-button @click="dialogCompletedRecordDetailsVisible = false">取 消</el-button>
                <el-button type="primary" @click="dialogCompletedRecordDetailsVisible = false">确 定</el-button>
              </div>
            </el-dialog>
          </td>
        </tr>
  ...
  export default {
    data() {
      return {
        ...
        dialogCompletedRecordDetailsVisible: false,
    ...
    methods: {
      ...
      deletedViewDetails(id, type) {
        ...
        this.$axios.post(this.url + '/borrowRecordInfo?java', {
          ...
          this.dialogCompletedRecordDetailsVisible = true;
        }).catch(err => {
          console.log(err);
        })
      },
